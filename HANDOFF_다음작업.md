...
## 10. 작업 결과 (2026-07-08, Claude — 2차 편집기 1군 완료, 사장님 승인 하)

- **§6 편집기 1군** = 커밋 b94e8c0. 선택 모드(행 체크 호버 노출 → 하단바=선택 액션바: 활성/비활성·언제나 활성화 켬/끔·번역·활성화 키 추가[선택분 스코프]·일괄 삭제·해제) · 복제(편집 폼, 원본 바로 뒤 배치) · 배치 순서 ↑/↓(order 스왑, 동률이면 전체 재부여) · 검색&치환(본문·이름만, 정규식 옵션, 일치 수 미리보기, edits 오버레이+옛 번역 무효화). 전부 오버레이 = 원본 불변·초안 자동 보존.
